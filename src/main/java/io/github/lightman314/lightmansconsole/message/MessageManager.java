	...
	public static class MessageEntry
	{
		...
		public String format(Object... format)
		{
			...
			for(int i = 0; i < formatKeys.size() && i < format.length; ++i)
			{
				...
				if(format[i] instanceof ITextComponent) //Check if it's a text component, and if so, run .getString() instead of .toString();
					formatText = ((ITextComponent)format[i]).getString();
		...
		public ITextComponent formatComponent(Object... format)
		{
			return new StringTextComponent(format(format));
		}
